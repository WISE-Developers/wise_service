...
if (DEBUG)
    console.log({ BUILDER_HOST, BUILDER_PORT });
...
function setSomeDefaults() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield modeller.globals.WISELogger.getInstance().info("Grabbing the jDefaults.");
        jDefaults = yield new modeller.defaults.JobDefaults().getDefaultsPromise();
        yield modeller.globals.WISELogger.getInstance().info("We have the jDefaults.");
        // console.log(jDefaults);
        let zoneCache = yield modeller.globals.Timezone.getTimezoneNameListPromise();
...
modeller.globals.WISELogger.getInstance().setLogLevel(modeller.globals.WISELogLevel.VERBOSE //  1 
...
const fireModel = (timeZoneObj, fireID, lat, lon, ele, fireGeom, modelDataset, weather, cffdrs, jobInfo, ...args) => {
    ...
    return new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
        var scenarioStartString = (yield weather.firstDateLocal.split(" ")[0]) + "T13:00:00";
        var scenarioEndString = (yield luxon.DateTime.fromISO(weather.firstDateLocal.split(" ")[0] + "T13:00:00").plus({ hours: 72 }).toISODate()) + "T13:00:00";
        var scenarioThreeDayEndString = (yield luxon.DateTime.fromISO(weather.firstDateLocal.split(" ")[0] + "T13:00:00").plus({ hours: 72 }).toISODate()) + "T13:00:00";
        yield modeller.globals.WISELogger.getInstance().info("WorstStart: " + scenarioStartString);
        yield modeller.globals.WISELogger.getInstance().info("WortEnd: " + scenarioEndString);
        yield modeller.globals.WISELogger.getInstance().info("3dayStart: " + scenarioStartString);
        yield modeller.globals.WISELogger.getInstance().info("3dayEnd: " + scenarioThreeDayEndString);
        var bcStartDate = yield weather.firstDateLocal.split(" ")[0];
        var bcDay1Date = yield luxon.DateTime.fromISO(weather.firstDateLocal.split(" ")[0] + "T13:00:00").plus({ hours: 24 }).toISODate();
        var bcDay2Date = yield luxon.DateTime.fromISO(weather.firstDateLocal.split(" ")[0] + "T13:00:00").plus({ hours: 48 }).toISODate();
        var bcDay3Date = yield luxon.DateTime.fromISO(weather.firstDateLocal.split(" ")[0] + "T13:00:00").plus({ hours: 72 }).toISODate();
        var bcDay4Date = yield luxon.DateTime.fromISO(weather.firstDateLocal.split(" ")[0] + "T13:00:00").plus({ hours: 96 }).toISODate();
        yield modeller.globals.WISELogger.getInstance().info("BurningConditionsStart: " + bcStartDate);
        yield modeller.globals.WISELogger.getInstance().info("BurningConditionsDay1: " + bcDay1Date);
        yield modeller.globals.WISELogger.getInstance().info("BurningConditionsDay2: " + bcDay2Date);
        yield modeller.globals.WISELogger.getInstance().info("BurningConditionsDay3: " + bcDay3Date);
        yield modeller.globals.WISELogger.getInstance().info("BurningConditionDay4End: " + bcDay4Date);
        yield modeller.globals.WISELogger.getInstance().info("Building WISE job.");
        //set this to the location of the test files folder.
        // console.log(modeller.wise)
        var prom = yield new modeller.wise.WISE();
        var projectionFile = (yield modelDataset.path) + modelDataset.prj;
        // console.log('Proj File used:', projectionFile);
        yield prom.setProjectionFile(projectionFile);
        yield prom.setElevationFile(modelDataset.path + modelDataset.elevation);
        yield prom.setFuelmapFile(modelDataset.path + modelDataset.fuels);
        yield prom.setLutFile(modelDataset.path + modelDataset.lut);
        yield prom.setTimezoneByValue(timeZoneObj.wise);
        var spotwxWeatherStation = yield prom.addWeatherStation(ele, new modeller.globals.LatLon(lat, lon));
        var header = yield "HOURLY,HOUR,TEMP,RH,WD,WS,PRECIP";
        var forecastLines = yield weather.records.map((r) => {
            return `${r.localDate}, ${r.localHour}, ${r.temp}, ${r.rh}, ${r.wd}, ${r.ws}, ${r.precip}`;
        });
        yield forecastLines.unshift(header);
        // console.log("forecast", forecast)
        var spotwxAttachment = yield prom.addAttachment('spotwx_forecast.txt', forecastLines.join('\n'));
        ...
        if (cffdrs.cffdrs.ffmc === null || cffdrs.cffdrs.dmc === null || cffdrs.cffdrs.dc === null) {
            yield console.log("WARNING: detected a null value in cffdrs:", cffdrs.cffdrs);
        ...
        if (cffdrs.cffdrs.ffmc <= 0 || cffdrs.cffdrs.dmc <= 0 || cffdrs.cffdrs.dc <= 0) {
            yield console.log("WARNING: detected an odd (less than zero) value in cffdrs:", cffdrs.cffdrs);
            cffdrs.cffdrs.ffmc = 85;
            cffdrs.cffdrs.dmc = 6;
            cffdrs.cffdrs.dc = 15;
            console.log("WARNING: substituting defaults in cffdrs:", cffdrs.cffdrs);
        }
        var spotwxWeatherStream = yield spotwxWeatherStation.addWeatherStream(String(spotwxAttachment), 94.0, 17, modeller.wise.HFFMCMethod.LAWSON, cffdrs.cffdrs.ffmc, cffdrs.cffdrs.dmc, cffdrs.cffdrs.dc, parseFloat(cffdrs.cffdrs.rn24), weather.firstDateLocal.split(" ")[0], weather.lastDateLocal.split(" ")[0]);
        ...
        if (fireGeom.geometry && fireGeom.geometry.type && fireGeom.geometry.type == 'Point') {
            ...
            var latLongObj = yield new modeller.globals.LatLon(lat, lon);
            // add it to the model
            ...
            //     );
            // })
            var mainIgnition = yield prom.addPointIgnition(latLongObj, 
        ...
        let threeDayScenarioOutputKml = prom.addOutputVectorFileToScenario(modeller.wise.VectorFileType.KML, '3dayModelScenario.kml', scenarioStartString, scenarioThreeDayEndString, threeDayScenario);
        ...
        let worstCaseScenarioOutputKml = prom.addOutputVectorFileToScenario(modeller.wise.VectorFileType.KML, 'WorstCaseModelScenario.kml', scenarioStartString, scenarioEndString, worstScenario);
        ...
        yield setSummaryOptions(threeDayScenarioOutputSummary);
        ...
        yield setSummaryOptions(worstCaseScenarioOutputSummary);
        ...
        let threeDayScenarioOutputJSON = prom.addOutputVectorFileToScenario(modeller.wise.VectorFileType.SHP, '3dayModelScenario.json', scenarioStartString, scenarioThreeDayEndString, threeDayScenario);
        ...
        let worstCaseScenarioOutputJSON = prom.addOutputVectorFileToScenario(modeller.wise.VectorFileType.SHP, 'WorstCaseModelScenario.json', scenarioStartString, scenarioEndString, worstScenario);
        ...
        if (prom.isValid()) {
            ...
            let wrapper = yield prom.beginJobPromise();
            ...
            var fireModel = yield { model: prom, jobName: jobName };
            // shell out and run WISE
            var jobFile = (yield remotejobsFolder) + fireModel.jobName + "/job.fgmj";
            var validateFile = (yield remotejobsFolder) + fireModel.jobName + "/validation.json";
            var threeDaySummaryFile = (yield remotejobsFolder) + fireModel.jobName + "/Outputs/threeDaySummary.txt";
            var threeDayKml = (yield remotejobsFolder) + fireModel.jobName + "/Outputs/3dayModelScenario.kml";
            const shellValidateString = yield `${wiseExec} --validate ${jobFile}`;
            const shellOutAndValidate = (shellValidateString, validateFile) => {
                return new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
                    try {
                        let validation = yield shellOut(shellValidateString);
                        console.log("validation Process worked", validation);
                        yield modeller.globals.WISELogger.getInstance().info(`validation Process: Worked`);
                        ...
                        if (fs.existsSync(validateFile)) {
                            ...
                            yield modeller.globals.WISELogger.getInstance().info(`validation File: exists`);
                            fs.readFile(validateFile, 'utf8', (error, bestData) => __awaiter(void 0, void 0, void 0, function* () {
                                let dataObj = JSON.parse(bestData);
                                if (error) {
                                    console.log("validate error", error);
                                    yield modeller.globals.WISELogger.getInstance().info(`error: ${error.message}`);
                                    resolve(false);
                                }
                                if (dataObj.valid == true) {
                                    // console.log("bestCase:", bestData.toString());
                                    console.log("validation passed");
                                    yield modeller.globals.WISELogger.getInstance().info(`validation passed`);
                                    resolve(true);
                                }
                                else {
                                    ...
                                    yield modeller.globals.WISELogger.getInstance().info(`validation failed ${dataObj.load_warnings}`);
                                    resolve(false);
                                }
                            }));
                        }
                        ...
                        console.log(error);
                        yield modeller.globals.WISELogger.getInstance().info(`Validation Process Broken: ${error.message}`);
                        resolve(false);
                    }
                }));
            };
            function shellOutAndModel(modelCommandString, summaryFile, fireID) {
                return new Promise((resolve, reject) => {
                    ...
                    const modellingJob = (0, child_process_1.spawn)(modelCommandString, { shell: true });
            ...
            try {
                let jobfileValidation = yield shellOutAndValidate(shellValidateString, validateFile);
                console.log("Validation complete...");
                if (!jobfileValidation) {
                    console.log("Validation failed...");
                    yield modeller.globals.WISELogger.getInstance().info(`Validation failed...`);
                    resolve(false);
                }
                else {
                    modeller.globals.WISELogger.getInstance().info(`Executing WISE Directly, ${fireModel.jobName} `);
                    try {
                        const shellCommandString = yield `${wiseExec} ${jobFile}`;
                        console.log("shellCommandString", shellCommandString);
                        let mShellRes = yield shellOutAndModel(shellCommandString, threeDaySummaryFile, fireID);
                        console.log("Modelling complete...");
                        if (!mShellRes) {
                            console.log("Modelling failed...");
                            yield modeller.globals.WISELogger.getInstance().info(`Modelling failed...`);
                            resolve(false);
                        }
                        else {
                            console.log("Modelling passed...");
                            yield modeller.globals.WISELogger.getInstance().info(`Modelling passed...`);
                            resolve(mShellRes);
                        }
                    }
                    catch (error) {
                        ...
                        console.log(error);
                        yield modeller.globals.WISELogger.getInstance().info(`Modelling Process Broken: ${error.message}`);
                        resolve(false);
                    }
                }
            }
...
const getElevationByLatLon = (latitude, longitude) => {
...
const easyPublicModel = (liveFire, jobInfo) => {
    ...
    return new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
        const fireID = yield liveFire.properties.datasetName;
        const datasetName = yield liveFire.properties.datasetName;
        const datasetPath = yield {
        ...
        const ignitionObjectRaw = yield JSON.parse(fs.readFileSync(datasetPath.path + ignitionFile, 'utf8'));
        const ignitionObject = (yield (ignitionObjectRaw.coordinates)) ? turf.feature(ignitionObjectRaw) : ignitionObjectRaw.geometry;
        console.log('ignitionObject', ignitionObject);
        const timezoneObject = yield JSON.parse(fs.readFileSync(datasetPath.path + timeZoneFile, 'utf8'));
        console.log('timezoneObject', timezoneObject);
        const cffdrsObject = yield JSON.parse(fs.readFileSync(datasetPath.path + cffdrsFile, 'utf8'));
        console.log('cffdrsObject', cffdrsObject);
        console.log('liveFire', liveFire);
        liveFire.geometry = (ignitionObject.coordinates) ? yield ignitionObject : yield ignitionObject.geometry;
        console.log("After adding geometry");
        const fireGeom = yield liveFire;
        console.log(fireGeom);
        if (fireGeom.type && fireGeom.type == 'Point') {
            console.log("we have point");
            var lat = yield fireGeom.coordinates[1];
            var lon = yield fireGeom.coordinates[0];
            ...
            let centroid = yield getCentriodFromGeom(fireGeom);
            ...
            console.log("we have point");
            var lat = yield fireGeom.geometry.coordinates[1];
            var lon = yield fireGeom.geometry.coordinates[0];
            var ignitionType = 'point';
        }
        else {
            console.log('we have a poly, need a centroid');
        ...
        let ele = (yield getElevationByLatLon(lat, lon)) || 100;
        //get forecast
        console.log("get weather for the fire");
        let weather = yield readWxService.readWxByFireID(fireID, lat, lon);
        ...
        //         localHour: '00'
        //       }]
        let noonCheck = yield weather.firstDateLocal.split(" ")[1];
        let dayCheck = yield weather.firstDateLocal.split(" ")[0];
        ...
        if (dayCheck == todayString) {
            ...
            if (noonCheck > 12) {
                ...
                console.log(weather);
                weather = yield weather.records.slice(1, 25);
                console.log(weather);
            }
            else {
        ...
        let rainObj = yield JSON.parse(fs.readFileSync(datasetPath.path + rainFile, 'utf8'));
        let rn12 = rainObj.rain;
        const cffdrs = cffdrsObject;
        cffdrs.cffdrs.rn24 = yield rn12;
        ...
        yield console.log("run the model....", { fireID, lat, lon, ele, fireGeom, datasetPath, weather, cffdrs, jobInfo });
        const model = yield fireModel(timezoneObject, fireID, lat, lon, ele, fireGeom, datasetPath, weather, cffdrs, jobInfo);
        yield console.log("model run complete....");
        resolve({
            model: model,
            inputs: {
